...
const TYPE = "Ref2VASequenceWorkbenchV01";
...
app.registerExtension({
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      if (!dataWidget || this.ref2vaSequenceReady) return result;
      ...
      root.className = "ref2va-sequence-v01";
      setStyle(root, "box-sizing:border-box;width:100%;min-width:520px;color:#e8f1fc;font:13px system-ui;background:#101824;border:1px solid #314b6d;border-radius:10px;padding:10px;");
      ...
      const render = () => {
        ...
        const header = setStyle(document.createElement("div"), "display:flex;gap:8px;align-items:center;margin-bottom:9px;");
        ...
        const body = setStyle(document.createElement("div"), "display:grid;grid-template-columns:178px minmax(0,1fr);gap:10px;");
        const directory = setStyle(document.createElement("div"), "background:#121f2d;border:1px solid #2f4965;border-radius:7px;padding:6px;max-height:245px;overflow-y:auto;");
        ...
        timeline.shots.forEach((shot, index) => {
          ...
          const item = button("", `display:block;width:100%;text-align:left;margin:0 0 5px;padding:7px;border-radius:6px;cursor:pointer;color:#eaf5ff;background:${selected ? "#244568" : "#182535"};border:1px solid ${selected ? "#66b7ff" : "#31485f"};`);
        ...
        const marker = setStyle(document.createElement("div"), "display:flex;justify-content:space-between;align-items:center;margin:1px 0 6px;color:#86c9ff;font-size:12px;");
        ...
        const prompt = setStyle(document.createElement("textarea"), "box-sizing:border-box;width:100%;height:136px;resize:vertical;background:#0d141e;border:1px solid #406084;color:#f3f7ff;border-radius:6px;padding:8px;line-height:1.45;");
        ...
        footer.append(document.createTextNode("v0.1：单镜头运行 · 当前提示词和时长已输出到主流程"));
      ...
      widget.computeSize = () => [620, 312];
      this.setSize([640, 382]);
      render();
      sync();
      return result;
    };
